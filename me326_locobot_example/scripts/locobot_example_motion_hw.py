...
#### note for learning:
Node -- ROS term for an excutable that is connected to the ROS network
...
class LocobotExample(object):
    ...
    def mobile_base_callback(self, data):
        ...
        Kp_angle_err = 2 #gain for angular error (here a scalar because we are only rotating about the z-axis)
        ...
        if err_magnitude < self.goal_reached_error:
            if self.counter == 0:
                ...
                self.t = seconds - self.t_init
                rx = 0.5 * np.cos(self.t * 2 * np.pi / 10)
                ry = 0.5 * np.sin(self.t * 2 * np.pi / 10)
                self.target_pose.position.x = rx
                self.target_pose.position.y = ry
